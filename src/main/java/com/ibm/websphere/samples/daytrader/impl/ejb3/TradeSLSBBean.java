...
import com.ibm.websphere.samples.daytrader.microservices.order.HoldingCreatedMessage;
import com.ibm.websphere.samples.daytrader.microservices.order.HoldingRemovedMessage;
import com.ibm.websphere.samples.daytrader.microservices.order.HoldingResponse;
import com.ibm.websphere.samples.daytrader.microservices.order.MessageType;
import com.ibm.websphere.samples.daytrader.microservices.order.OrderCreateInput;
...
import com.ibm.websphere.samples.daytrader.microservices.order.OrderResponseToOrderDataBeanConvertor;
import com.ibm.websphere.samples.daytrader.microservices.order.OrderService;
import com.ibm.websphere.samples.daytrader.microservices.order.OrderStatus;
import com.ibm.websphere.samples.daytrader.microservices.order.OrderType;
import com.ibm.websphere.samples.daytrader.microservices.order.QueryOrderInput;
import com.google.gson.Gson;
import com.ibm.websphere.samples.daytrader.aws.sns.SNSProducer;
...
public class TradeSLSBBean implements TradeServices {
    ...
    public OrderDataBean buy(String userID, String symbol, double quantity, int orderProcessingMode) {
        ...
        try {
            ...
            order = createOrderV2(account, quote, holding, "buy", quantity);
 
            // UPDATE - account should be credited during completeOrder
            /*
            //Commented for the experiment
    ...
    public OrderDataBean sell(final String userID, final Integer holdingID, int orderProcessingMode) {
        ...
        try {
            ...
            order = createOrderV2(account, quote, holding, "sell", quantity);
    ...
    public Collection<OrderDataBean> getClosedOrders(String userID) {
        ...
        try {
            ...
            QueryOrderInput input = new QueryOrderInput(null,userID, Arrays.asList(status));
      	  	...
      	  	for (OrderResponse orderResponse : orderResponses) {
      	  	ordersResponse.add(convertor.convertToOrderDataBean(orderResponse));
    		}
    ...
    public OrderDataBean createOrderV2(AccountDataBean account, QuoteDataBean quote, HoldingDataBean holding, String orderType, double quantity) {
        OrderDataBean order;
        ...
        try {
            ...
            account = entityManager.find(AccountDataBean.class, account.getAccountID() );

            OrderCreateInput orderInput = new OrderCreateInput(
      			  account.getAccountID().toString(),
      			  account.getProfile().getUserID(),
                    quote.getSymbol().toString(),
                    holding!=null?holding.getHoldingID().toString():null,
                    quantity,
      	  ...
      	  OrderResponseToOrderDataBeanConvertor convertor=new OrderResponseToOrderDataBeanConvertor();
      	  return convertor.convertToOrderDataBean(orderResponse);
	...
	public void orderCreated(OrderResponse order) {
        ...
        try {
            ...
            AccountProfileDataBean profile = entityManager.find(AccountProfileDataBean.class, order.getUserId());
            AccountDataBean account = profile.getAccount();
            QuoteDataBean quote = entityManager.find(QuoteDataBean.class, order.getQuoteId());
            if(order.getType()==OrderType.BUY) {
            	 
                // UPDATE - account should be credited during completeOrder
                BigDecimal price = quote.getPrice();
                BigDecimal orderFee = BigDecimal.valueOf(order.getOrderFee());
                BigDecimal balance = account.getBalance();
                total = (new BigDecimal(order.getQuantity()).multiply(price)).add(orderFee);
                account.setBalance(balance.subtract(total));
                final Integer orderID=order.getId();
                
                //Create holding
            	HoldingDataBean newHolding = createHolding(account, quote, order.getQuantity(), price);

                //Notify completation
            	HoldingCreatedMessage message=new HoldingCreatedMessage();
                message.setMessageType(MessageType.HOLDING_CREATED);
                message.setHolding(new HoldingResponse(String.valueOf(order.getId()),String.valueOf(newHolding.getHoldingID().toString())));
                
            	String jsonMessage = gson.toJson(message);
                
                snsProducer.sendMessage(message);
                
                Log.error("Notify HOLDING_CREATED");
            }else {
            	HoldingDataBean holding = entityManager.find(HoldingDataBean.class, Integer.parseInt(order.getHoldingId()));

                if (holding == null) {
                    Log.debug("TradeSLSBBean:sell User " + order.getUserId() + " attempted to sell holding " + order.getHoldingId() + " which has already been sold");

                    Log.error("Notify ORDER_CANCELED");
                ...
                //Notify removed
                HoldingRemovedMessage message=new HoldingRemovedMessage();
                message.setMessageType(MessageType.HOLDING_REMOVED);
                message.setHolding(new HoldingResponse(String.valueOf(order.getId()),String.valueOf(holding.getHoldingID())));
                
                
                snsProducer.sendMessage(message);
                Log.error("Notify HOLDING_REMOVED");
            }
            
        } catch (Exception e) {
            Log.error("TradeSLSBBean:buy(" + order.getUserId() + "," + order.getQuoteId() + "," + order.getQuantity() + ") --> failed", e);
            /* On exception - cancel the order */
            // TODO figure out how to do this with JPA
            // if (order != null) order.cancel();
            throw new EJBException(e);
